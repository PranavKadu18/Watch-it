    ...
    >
      ...
      <div className="flex w-full min-h-[50vh] text-white">
            ...
            <div className="pl-12">
                <div className="flex items-end gap-1">
                    <h1 className="font-bold text-4xl">{info.details.name || info.details.title || info.details.original_title || info.details.original_name }</h1>
                    <h1 className="font-semibold text-xl">({info.details.first_air_date.split("-")[0]})</h1>
                </div>
              ...
              <Link to={`/tv/details/${info.external_id.id}/trailer`} className="px-4 py-1 mt-4 w-fit bg-[#6556CD] rounded-md flex items-center gap-2">
      ...
      <div className="box w-full min-h-[38.5vh]   pb-7 overflow-x-auto overflow-y-hidden">
          ...
          <div className=' relative w-full min-h-[42vh] flex gap-2 items-center justify-start overflow-visible '>
              {info.details.seasons.map((elem, idx) => <S data={elem} /> )}
          </div>
      </div>
      ...
      (
        <div className='box w-full mt-4 min-h-[38.5vh]   pb-7 overflow-x-auto overflow-y-hidden'>
            ...
            <div className=' relative w-full min-h-[42vh] flex gap-2 items-center justify-start overflow-visible '>
                {info.recommendations.map((elem, idx) => <Card key={idx} type={"tv"} info={elem} />)}
            </div>
        </div>
            ...
            <div className='box w-full mt-4 min-h-[38.5vh]   pb-7 overflow-x-auto overflow-y-hidden'>
                ...
                <div className=' relative w-full min-h-[42vh] flex gap-2 items-center justify-start overflow-visible '>
                    {info.similar.map((elem, idx) => <Card key={idx} type={"tv"} info={elem} />)}
                </div>

            </div> : 
            <div className="w-full h-[38.5vh] flex items-center justify-center">No Information Available</div>
        )}
